    ...
    def formatday(self, day, weekday):
        ...
        if day == 0:
            ...
            if day in self.active_days:
                day_url = reverse('overview_day',
                                  args=[self.current_day.replace(day=day).isoformat()]) + '?' + urlencode({
                    'filter': self.model._meta.verbose_name.lower(),
                    self.model._meta.verbose_name.lower(): self.model.pk
                })
